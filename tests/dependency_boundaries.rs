...
#[test]
fn ipc_modules_do_not_import_obs_implementation_modules() {
    let root = workspace_root();
    let violations = rust_files(&root.join("src/ipc"))
        .into_iter()
        .flat_map(|path| disallowed_imports(&root, &path, imports_obs_implementation))
        .collect::<Vec<_>>();

    assert!(
        violations.is_empty(),
        "IPC modules must not import OBS implementation modules:\n{}",
        violations.join("\n")
    );
}

#[test]
fn domain_does_not_import_ipc_protocol_or_obs_client_types() {
    let root = workspace_root();
    let violations = rust_files(&root.join("src/domain"))
        .into_iter()
        .flat_map(|path| {
            disallowed_imports(&root, &path, |import| {
                imports_ipc_protocol(import) || imports_obs_client(import)
            })
        })
        .collect::<Vec<_>>();
    ...
    assert!(
        violations.is_empty(),
        "Domain modules must not import IPC protocol wire types or OBS client implementation types:\n{}",
        violations.join("\n")
    );
}

#[test]
fn import_matchers_catch_direct_and_grouped_boundary_violations() {
    let obs_imports = [
        "use crate::obs::client::ObsEvent;",
        "use crate::obs::{client::ObsEvent};",
        "use crate::{config::Config, obs::client::ObsEvent};",
        "use obsctl_rs::obs::client::ObsClient;",
        "use obsctl_rs::{config::Config, obs::{client::ObsEvent}};",
        "use obs::client::ObsEvent;",
        "use obs::{client::ObsEvent};",
    ];

    for import in obs_imports {
        let normalized = normalize_import(import);
        assert!(
            imports_obs_implementation(&normalized),
            "should catch OBS implementation import: {import}"
        );
    }
    ...
    let ipc_imports = [
        ...
        "use crate::ipc::{protocol::ObsEventPayload};",
        "use crate::{domain::result::Result, ipc::protocol::ServerMessage};",
        "use obsctl_rs::ipc::protocol::ObsEventPayload;",
        "use obsctl_rs::{domain::result::Result, ipc::{protocol::ServerMessage}};",
        "use ipc::protocol::ObsEventPayload;",
        "use ipc::{protocol::ServerMessage};",
    ];

    for import in ipc_imports {
        let normalized = normalize_import(import);
        assert!(
            imports_ipc_protocol(&normalized),
            "should catch IPC protocol import: {import}"
        );
    }
}
...
fn disallowed_imports(
    ...
    extract_imports(&production_source(&source))
        .into_iter()
        .filter(|import| is_disallowed(import))
...
fn imports_obs_implementation(import: &str) -> bool {
    imports_crate_module(import, "obs")
        || imports_package_module(import, "obsctl_rs", "obs")
...
fn imports_ipc_protocol(import: &str) -> bool {
    imports_crate_nested_module(import, "ipc", "protocol")
        || imports_package_nested_module(import, "obsctl_rs", "ipc", "protocol")
        || imports_bare_nested_module(import, "ipc", "protocol")
}

fn imports_crate_module(import: &str, module: &str) -> bool {
    import == format!("crate::{module}")
        || import.starts_with(&format!("crate::{module}::"))
        || import.starts_with(&format!("crate::{module}::{{"))
        || import.starts_with(&format!("crate::{{{module}::"))
        || import.starts_with(&format!("crate::{{{module}::{{"))
        || import.contains(&format!(",{module}::"))
        || import.contains(&format!(",{module}::{{"))
}

fn imports_package_module(import: &str, package: &str, module: &str) -> bool {
    import == format!("{package}::{module}")
        || import.starts_with(&format!("{package}::{module}::"))
        || import.starts_with(&format!("{package}::{module}::{{"))
        || import.starts_with(&format!("{package}::{{{module}::"))
        || import.starts_with(&format!("{package}::{{{module}::{{"))
        || import.contains(&format!(",{module}::"))
